...
{
	UART_addr -> UART_UCSRA =U2X_bit;
	
	uint16_t UBBR_value;
	UBBR_value = (uint16_t)(((F_CPU/F_DIV)/(8UL*(2-U2X_bit)*baud_rate))-1);
	
	UART_addr -> UART_UBRRH = UBBR_value >> 8;//divide by 256 
	UART_addr -> UART_UBRRL = UBBR_value &=0xFF;//% 256
	
	UART_addr-> UART_UCSRC = (asynch_mode|no_parity|one_stop_bit|Eight_Bit_Data); //setting up the size of message
	UART_addr -> UART_UCSRB = (transmit_enable|receive_enable); //enable transmit and receive
	return 0;
}
